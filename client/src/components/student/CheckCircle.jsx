...
export default function CheckCircle({
  ...
  return (
    <button
      ...
      aria-label={label}
      title={disabled ? 'Locked — complete the previous lesson first' : completed ? 'Completed' : 'Mark as complete'}
      className={`relative grid h-8 w-8 shrink-0 place-items-center rounded-full border transition-all duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-default/60 disabled:cursor-not-allowed ${
        disabled
          ? 'border-[rgba(143,170,205,0.18)] bg-[rgba(9,17,27,0.4)]'
          : completed
